			...
			if (gameState == GAME_STATE_INIT || gameState > GAME_STATE_ACTIVE && input[INPUT_KEY] == 1) {
				...
				for (int k = HEIGHT_MAP; k <= TREE_MAP; k++) {
					...
					for (int z = 0; z < 400; z++) {
						...
						x = (int)(Math.random() * (MAP_SIZE+1));
						y = (int)(Math.random() * (MAP_SIZE+1));
						// feature height and radius
						int size = (int)(Math.random() * (MAP_SIZE/8));
						for (i = x-size; i <= x+size; i++) {
							for (j = y-size; j <= y+size; j++) {
								if (i >= 0 && i < MAP_SIZE+1 && j >= 0 && j < MAP_SIZE+1) {
									// feature impact is reduced based on distance from feature centre point
									world[i][j][k] += size - Math.max(Math.abs(x-i),  Math.abs(y-j));
								}
							}
						}
					}
					...
					for (i = 0; i < MAP_SIZE+1; i++) {
						for (j = 0; j < MAP_SIZE+1; j++) {
							world[i][j][k] = Math.max(0, 10 + (world[i][j][k] - avg)/10 * 10);
						}
					}
				}
				...
				for (i = 0; i < MAP_SIZE; i++) {
					for (j = 0; j < MAP_SIZE; j++) {
						...
						} else if (centreHeight > 10 && Math.random()*3000 < centreHeight) {
				...
				for (i = 0; i < 9; i++) {
					...
					for (j = 0; j < 256; j++) {
						x = 12 + i%3*38 + (int)(Math.random()*28);
						y = 12 + i/3*38 + (int)(Math.random()*28);
...
//										entity[TIMER] = DELAY_WORKER_RESOURCE;
								...
								case TASK_GATHER_RESOURCE:
									if (entity[HOME_TYPE] == TYPE_FARM) {
										entities[world[entityTiley][entityTilex][CONTENT_ID]][TIMER] = DELAY_HARVEST_CROP + (int)(Math.random()*DELAY_HARVEST_CROP);
									}
...
//								if (entity[OWNER] > -1) { // make AI play against itself
									...
									if (entity[TOTAL_FOOD] > 0 && levelTime >= entity[INVADE_VILLAGE_AI_TIMER] && entity[TOTAL_SOLDIERS] > 1) {
										...
										for (j = 0; j < villagesTotal; j++) {
											if (j != i) {
												entity0 = entities[j];
//												int dist = Math.max(Math.abs(entity[X]-entity0[X]), Math.abs(entity[Y]-entity0[Y]))/MAP_SIZE;
												int armyDifference = entity[TOTAL_SOLDIERS] - entity0[TOTAL_SOLDIERS];
												int score = entity[TOTAL_FOOD]/2 + armyDifference 
														- Math.max(Math.abs(entity[X]-entity0[X]), Math.abs(entity[Y]-entity0[Y]))/MAP_SIZE;
									...
									if (entity[INVADE_VILLAGE_STATE] == 0 && entity[TOTAL_WOOD] >= COST_BUILDING 
										...
										if (bestScore > 0) {
											...
											for (int dist = 3; dist < 20; dist+=2) {
												x = entityTilex + 2*((int)(Math.random()*dist)-dist/2);
												y = entityTiley + 2*((int)(Math.random()*dist)-dist/2);
...
//															TYPE_CROPS, 
//															entitiesTotal,
//															(int)(Math.random()*DELAY_HARVEST_CROP),
//															TASK_IDLE,
...
//													};
													...
													entity0[ENTITY_ID] = entitiesTotal;
													entity0[TIMER] = (int)(Math.random()*DELAY_HARVEST_CROP);
